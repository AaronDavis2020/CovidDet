...
class Runthread(QtCore.QThread): # 定义子线程，只负责检测病人序列，控件行为由主线程负责
    ...
    def run(self):
        ...
        glm = GenLungMask(self.case_path) # dcms path & target path
        dcm_list, dataset = glm.genLung()
        dcm_list = dcm_list[int(len(dcm_list)*0.1) : int(len(dcm_list)*0.83)] # truncate the slice sequence-head 10%,tail 17%
        dataset = dataset[int(len(dataset)*0.1) : int(len(dataset)*0.83)]
        inference = slicePredict(use_cuda, dcm_list, dataset)
        ...
        # desktop_path = os.path.join(os.path.expanduser("~"), 'Desktop') # 写入csv
        # name = ['Patient','Class']
...
class Ui_MainWindow(object):
    ...
    def runProgress(self): # progressBar
        ...
        for i in range(100):
            ...
            QThread.msleep(150) # 模拟处理进程，进度条逐渐增加
